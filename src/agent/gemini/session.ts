...
export class GeminiSession implements AgentSession {
  ...
  async send(prompt: string): Promise<void> {
    ...
    for await (const line of createLineIterator(child.stdout!)) {
      ...
      if (obj.type === "init" && obj.session_id) {
        ...
        this.currentEvents.push({ type: "tool_use", tool: obj.name ?? "tool", input: "" });
        ...
        this.currentEvents.push({ type: "error", message: obj.message ?? "Gemini error" });
      }
    }
  ...
  private buildArgs(): string[] {
    ...
    const modeFlags: Record<string, string[]> = {
      default: [], auto_edit: ["--approval-mode", "auto_edit"],
      yolo: ["-y"], plan: ["--approval-mode", "plan"],
    };
